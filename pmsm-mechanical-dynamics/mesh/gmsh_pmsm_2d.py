...
# ----------------------------------------------------------------------
# 4b) Curved (2nd-order) elements in the airgap
#
# Gmsh's mesh element order (straight-sided vs. curved) is a single
# global setting - there's no per-physical-group order in the standard
# API. Setting it to 2 makes Gmsh generate quadratic elements directly:
# any element edge lying ON a curved geometric boundary (both airgap
# circles at r_rotor/r_mst/r_stator_in, the rotor, the stator OD, the
# bolt holes) gets a mid-side node placed on the true circle instead of
# the straight chord. Edges away from curved boundaries (e.g. the square
# air box) are left effectively straight since there's no curvature to
# represent there. This is what gives you curved elements in the airgap
# specifically, at the cost of the whole mesh being 2nd order.
#
# HighOrderOptimize smooths/untangles the curved mid-side nodes - useful
# here since the airgap is only (r_stator_in - r_rotor) = 2 mm wide with
# a 0.6 mm element size, where naive curving can otherwise produce
# invalid (self-intersecting) elements.
# ----------------------------------------------------------------------
gmsh.option.setNumber("Mesh.ElementOrder", 2)
gmsh.option.setNumber("Mesh.HighOrderOptimize", 2)   # 2 = elastic + optimize smoothing
